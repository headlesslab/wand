...
- **What stays skipped**: only environment guards. `TestFonts` runs in a container only, `TestBinarySize` outside Windows and containers only, `TestProfileDir` with `-test-profile-dir` only, `TestLaunchXVFB` where `xvfb-run` is installed, `TestUserModeBrandedChrome` (the Confirmed fix for rod #1189, ticket #47) where `LookPath` finds branded Google Chrome, which alone refuses remote debugging on its default profile, and on Linux only with a display or `xvfb-run`; it runs on the Gate's `ubuntu-latest` job today and on `windows-latest` and `macos-latest` once the matrix of #54 lands. No test is skipped for flakiness, listens on a fixed port, or reaches the public internet: the pooled browsers, and the ones `g.launch` starts, run with `--host-resolver-rules` that resolve no host but loopback, so a test that tries fails on every machine. The Managed browser download is the one network access, taken only when no browser is found, before any browser starts.
