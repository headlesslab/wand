...
`launcher.NewUserMode()` drives a visible browser on a persistent profile wand owns, so that logins, extensions and settings survive between runs. The profile is `wand/user-mode` under the user's configuration directory (`$XDG_CONFIG_HOME` or `~/.config` on Linux, `~/Library/Application Support` on macOS, `%AppData%` on Windows), `launcher.DefaultUserModeDir`, made at the first launch; `Launcher.UserDataDir()` names another. The browser comes from Browser resolution like any other, so it is the Chrome you have installed unless you say otherwise. The first launch on the profile is the browser's first run, and wand passes `--no-first-run`: branded Chrome on Linux and macOS would otherwise hold the launch on a modal first-run dialog that nothing dismisses under automation.

User mode does not open Chrome's own profile, the one with your daily sign-ins: since Chrome 136 branded Google Chrome refuses remote debugging on its default profile and exits with "DevTools remote debugging requires a non-default data directory", which is what go-rod's user mode ran into (rod #1189); a launcher pointed at that directory with `UserDataDir()` gets the same refusal. Sign in once in the wand profile instead, and the session is there on the next run. The Orphan guard is off in User mode, so the browser outlives your program, and a launch finds a browser already listening on the port before starting another. After a browser the launcher started exits, `Launcher.Cleanup()` removes the profile like any user data directory; `Launcher.Kill()` leaves it. A launcher that found a browser already listening owns neither it nor the profile: its `Kill()` and `Cleanup()` have nothing to do and leave both alone.
