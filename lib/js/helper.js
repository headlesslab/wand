...
//     go test -run ^TestClick$ -- -wand=show,devtools
...
const functions = {
  ...
  waitLoad() {
    ...
    // The promise resolves with nothing, never with the load event: the
    // event goes back to Go by value, and a listener registered ahead of
    // this one may have given it a circular property (Bootstrap's event
    // delegation sets delegateTarget), which cannot be serialized.
    return new Promise((resolve, reject) => {
      if (isWin) {
