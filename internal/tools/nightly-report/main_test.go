...
func (f *fake) cli(token string, args ...string) ([]byte, error) {
...
const (
	redSuite = `{"name":"Tier 1 rerun linux/arm64 (Go stable)","conclusion":"failure",` +
...
func TestARedJobOpensItsIssue(t *testing.T) {
	...
	g.Has(strings.Join(created.args, " "), "--title Nightly: Tier 1 rerun linux/arm64 (Go stable) failed")
	...
	g.Has(body, "Tier 1 rerun linux/arm64 (Go stable)")
	...
	g.Has(out.String(), "opened an issue: Nightly: Tier 1 rerun linux/arm64 (Go stable) failed")
}
...
func TestTheSecondNightCommentsOnTheOpenIssue(t *testing.T) {
	...
	f := &fake{answers: map[string]string{
		"api":        jobsOf(redSuite),
		"issue list": `[{"number":123,"title":"Nightly: Tier 1 rerun linux/arm64 (Go stable) failed"}]`,
	}}
...
func TestOnlyTheExactTitleIsTheSameIssue(t *testing.T) {
	...
	f := &fake{answers: map[string]string{
		"api": jobsOf(redSuite),
		"issue list": `[{"number":7,"title":"Tier 1 rerun linux/arm64 (Go stable) failed"},` +
			`{"number":8,"title":"Nightly: Tier 1 rerun linux/amd64 (Go stable) failed"}]`,
	}}

	g.E(report(f.cli, opts, &strings.Builder{}))
...
func TestOneIssuePerJob(t *testing.T) {
	...
	g.Eq(titles, []string{
		"Nightly: Tier 1 rerun linux/arm64 (Go stable) failed",
...
func TestADryRunOpensNothing(t *testing.T) {
	...
	out := &strings.Builder{}
	g.E(report(f.cli, dry, out))

	g.Eq(f.count("issue create"), 0)
	g.Eq(f.count("issue comment"), 0)
	g.Has(out.String(), "would open an issue: Nightly: Tier 1 rerun linux/arm64 (Go stable) failed")
...
func TestReportRefusals(t *testing.T) {
	...
	for _, o := range []options{{run: "42"}, {repo: "headlesslab/wand"}} {
		g.Err(report((&fake{}).cli, o, &strings.Builder{}))
	}
	...
	g.Err(report((&fake{answers: map[string]string{"api": ""}}).cli, opts, &strings.Builder{}))

	// Something that is not the JSON the Actions API documents.
	g.Err(report((&fake{answers: map[string]string{"api": "<html>rate limited</html>"}}).cli, opts, &strings.Builder{}))
...
func TestOneJobFailingToReportLeavesTheNextReported(t *testing.T) {
	...
	err := report(f.cli, opts, &strings.Builder{})
	g.Err(err)
	g.Eq(f.count("issue create"), 2)
